...
function SelecionarPessoa() {
    return <Container><Wrapper>
        <div>
            ...
            <Heading as="h1">Junte-se à
                nossa&nbsp;comunidade<Line/></Heading>

            <MainParagraph>{selecionarPessoaData.description}</MainParagraph>
            <Buttons>
                <Button href="/selecionar-pessoa" btnType="primary">{selecionarPessoaData.buttonOne}</Button>
                <Button href="/selecionar-pessoa" btnType="secondary">{selecionarPessoaData.buttonTwo}</Button>
            </Buttons>
        </div>
    </Wrapper>
        <ImageWrapper>
            <PeopleImg fill={true} src={People} alt="Imagem de uma profissional da saúde orientando uma paciente"/>
        </ImageWrapper>
    </Container>
}
